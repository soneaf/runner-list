...
} from '/var/task/Pictures/Runner List/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Pictures/Runner List/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Pictures/Runner List')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Pictures/Runner List',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Pictures/Runner List/.netlify/dist/run/handlers/server.js')
